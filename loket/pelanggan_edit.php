...
if (isset($_SESSION['loket'])) {
	...
	if ($_GET['IDPelanggan']) {
		...
		$query = mysqli_query($konek, "SELECT * from pelanggan WHERE IDPelanggan = '{$IDPelanggan}'");
		foreach ($query as $row) {
	
			$IDPelanggan                =    $row['IDPelanggan'];
			$vNama                      =    $row['Nama'];
			$vNoMeter                   =    $row['NoMeter'];
			$vAlamat                    =    $row['Alamat'];
			$vKodeTarif                 =    $row['KodeTarif'];
		}
	}
...
<div style="width: 99.8%;display: flex;height: 110%;background: #54f785;position:fixed">	
		...
		<br><br>
			<h1 style="text-align: center;margin-bottom: 25px">Edit Data Pelanggan <?=@$IDPelanggan?></h1>
      		<form action="pelanggan_proses.php" method="POST">
				<table style="width: 60%;overflow-x: scroll;margin:auto" cellspacing="0">
					...
					<tr>
						...
						<td style="width: 65%;padding: 10px 20px"><input type="text" name="NoMeter" style="border:1px solid;border-radius:5px;font-weight:700;width: 100%;text-decoration: none;padding: 7px 10px;" required="true" autocomplete="off" value="<?=@$vNoMeter ?>"></td>
					</tr>
					<tr>
						<td style="width: 35%;padding: 10px 20px">Alamat</td>
						<td style="width: 65%;padding: 10px 20px"><input type="text" name="Alamat" style="border:1px solid;border-radius:5px;font-weight:700;width: 100%;text-decoration: none;padding: 7px 10px;" required="true" autocomplete="off" value="<?=@$vAlamat ?>"></td>
					</tr>
					<tr>
						<td style="width: 35%;padding: 10px 20px">Tarif</td>
                        <td style="width: 65%;padding: 10px 20px;padding-right: 0">
							<select name="KodeTarif" id="KodeTarif" style="border:1px solid;border-radius:5px;font-weight:700;width: 100%;text-decoration: none;padding: 7px 10px;" required="true" autocomplete="off">
								...
								<?php 
								 ...
								 ?>
								 <?php if ($row['KodeTarif'] == $vKodeTarif) { ?>
