...
SESSION_START();
	...
	if (isset($_SESSION['superadmin'])) {
		...
		$kodekecamatan = $_GET['kode_kecamatan'];
		$query = $konek->query("SELECT * FROM `kecamatan` WHERE kode_kecamatan = '".$kodekecamatan."'");
		$num = $query->num_rows;
...
<body>
        ...
        <div class="form-konten">
			...
			<h2>Detail Bidang <?= $row['nama_kecamatan'] ?> </h2>
            <br><br>
            <table style="width: 110%;overflow-x: scroll;margin-left: -25px" cellpadding="1" cellspacing="1" border="1px" >
					<tr>
						<td style="width: 30%;font-weight:700;padding: 10px 20px;text-align: center;font-size: 15px;background-color: white">Kode Kecamatan :</td>
						<td style="width: 40%;padding: 5px 20px;text-align: center;background-color: white"><?= $row['kode_kecamatan'] ?></td>
					</tr>
                    <tr>
                        <td style="width: 35%;font-weight:700;padding: 10px 20px;text-align: center;font-size: 15px;background-color: white">Nama Kecamatan :</td>
                        <td style="width: 65%;padding: 15px 20px;text-align: center;background-color: white"><?= $row['nama_kecamatan'] ?></td>
                    </tr>
				</table>
