...
if (isset($_SESSION['superadmin'])) {
	...
	if ($_GET['kode_industri']) {
		$kodeindustri = $_GET['kode_industri'];
		$query = mysqli_query($konek, "SELECT * FROM `ikm` WHERE `kode_industri` = '{$kodeindustri}'");
		foreach ($query as $row) {

			$kodeindustri               =   $row['kode_industri'];
    		$namaindustri              =   $row['nama_industri'];
    		$vnamapemilik               =   $row['nama_pemilik'];
    		$vkontak                    =   $row['kontak'];
    		$valamatindustri            =   $row['alamat_industri'];
    		$vkodebidang                =   $row['kode_bidang'];
		}
	}
            ...
            <br><br>
              ...
              <br>
				...
				<div>
					...
					<input type="text" name="nama_pemilik" required="true" style="font-weight:700;width: 70%;text-decoration: none;padding: 10px 10px;border-radius: 5px;border: 1px solid; margin-left: -12%; margin-top: -10px" autocomplete="off" value="<?= $vnamapemilik ?>">
				</div>
				<div>
					...
					<input type="text" name="kontak" required="true" style="font-weight:700;width: 40%;text-decoration: none;padding: 10px 10px;border-radius: 5px;border: 1px solid; margin-left: -42%; margin-top: -10px" autocomplete="off" value="<?= $vkontak ?>">
				</div>
				<div>
					...
					<input type="text" name="alamat_industri" required="true" style="font-weight:700;width: 70%;text-decoration: none;padding: 20px 10px;border-radius: 5px;border: 1px solid; margin-left: -12%; margin-top: -10px" autocomplete="off" value="<?= $valamatindustri ?>">
				</div>
				<div>
                <p style="width: 35%;padding: 10px 10px; margin-top: 5px; margin-left: 22px; margin-bottom: 5px;color:darkslategray; font-weight:bold;font-size: 20px;font-family:'Gill Sans', 'Gill Sans MT', Calibri, 'Trebuchet MS', sans-serif">Nama Bidang</p>
					<select name="kode_bidang" id="kode_bidang" style="border:1px solid;border-radius:5px;font-weight:700;width: 70%;text-decoration: none;padding: 7px 10px; margin-left: -12%; margin-top: -10px" required="true" autocomplete="off">
                        ...
                        <?php 
                            ...
                            ?>
                            <?php if ($row['kode_bidang'] == $vkodebidang) { ?>
